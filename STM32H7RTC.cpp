...
void STM32RTC::setTime(uint8_t _h, uint8_t _m, uint8_t _s, uint32_t _ss, uint8_t _pmAm, uint32_t _dayLightSaving)
{
...
RTC_DateTypeDef STM32RTC::getDate(uint8_t *_d, uint8_t *_m, uint8_t *_y, uint8_t *_weekday)
{
    RTC_DateTypeDef myDate = {0};
    HAL_RTC_GetDate(&hrtc, &myDate, RTC_FORMAT);
...
void STM32RTC::enableAlarmInterrupt(void (*_f)())
{
...
void STM32RTC::disableAlarm(uint32_t _alarm)
{
    HAL_RTC_DeactivateAlarm(&hrtc, _alarm);
}

void STM32RTC::disableAlarmInterrupt()
{
...
RTC_AlarmTypeDef STM32RTC::getAlarm()
{
    uint8_t dummy1;
    uint32_t dummy2;
